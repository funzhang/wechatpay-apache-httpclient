...
public class CertificatesManager {
  ...
  private class DefaultVerifier implements Verifier {
    ...
    public boolean verify(String serialNumber, byte[] message, String signature) {
      ...
      ConcurrentHashMap<BigInteger, X509Certificate> merchantCertificates =
          certificates.get(merchantId);
  ...
  private synchronized void downloadAndUpdateCert(
      String merchantId, Verifier verifier, Credentials credentials, byte[] apiV3Key)
      throws HttpCodeException, IOException, GeneralSecurityException {
    try (CloseableHttpClient httpClient =
        WechatPayHttpClientBuilder.create()
            .withCredentials(credentials)
            .withValidator(
                verifier == null ? (response) -> true : new WechatPay2Validator(verifier))
            .build()) {
      ...
      try (CloseableHttpResponse response = httpClient.execute(httpGet)) {
        ...
        if (statusCode == SC_OK) {
          Map<BigInteger, X509Certificate> newCertList =
              CertSerializeUtil.deserializeToCerts(apiV3Key, body);
